...
- Auction integration events carry the authoritative `tenantId`. Live Feed uses it for validation, tenant-aware Redis state/history, and Socket.IO rooms; the Operations Portal persists it with each activity record. Bidding Service tenant-facing HTTP routes enforce the authoritative persisted tenant status: Active permits normal access, Suspended is read-only, and Disabled is blocked while retained for global monitoring. Public anonymous Live Feed admission calls Bidding before each new room join, and accepted `TenantStatusChanged` events revoke already-connected public rooms for Disabled tenants. Client admission is mandatory in production and operationalized by the MT5.4 bootstrap runbook.
