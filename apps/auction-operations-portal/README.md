...
The Operations Portal is the independent system-administration boundary. Laravel tenant administrators no longer authenticate into this portal; system administrators sign in through the local `/login` flow.

After local authentication, the portal stores only minimized identity claims in its own short-lived HttpOnly cookie (`auction_operations_auth`). Logout clears that cookie and returns to `/login`. Direct system-admin credentials are owned by the Operations Portal account store.

## System-admin identity foundation

SYS1 adds a separate `system_admin_users` account store owned by this Operations/System Admin side. It is intentionally not connected to the Laravel tenant-user table or the Bidding Service database. Accounts use salted adaptive ASP.NET password hashes, stable opaque `SubjectId` values, and the dedicated `SystemAdministrator` role. That role derives only `system.monitor`, `livefeed.admin`, and `system.diagnostics`; it does not imply the tenant-side `auction.manage`, `auction.bid`, or `auction.buy` permissions.

The independent `SystemAdminAuth` configuration issues short-lived RS256 tokens for explicit service audiences, including `live-feed-admin`. It uses a configurable issuer, explicit `KeyId`, private-key path, and a 60–600 second token lifetime. Private signing keys remain on the system-admin side; Live Feed receives public verification material only. Non-development startup fails when required issuer configuration or the private key is missing.
...
SYS2 adds `/login` to the Operations Portal. It verifies a local `SystemAdminUser` account and creates the short-lived, HttpOnly `auction_operations_auth` cookie. The portal session is an interactive human session (20 minutes by default), distinct from the 60–600 second server-to-service JWT foundation. Login and logout are POST-based and antiforgery-protected; failed login responses are intentionally generic.
...
The current explicit DI map is intentional: singleton `AuctionActivityConsumer`, `IActivityNotificationPublisher`, `IntegrationEventMapper`, and `RabbitMqTopology`; scoped `AuctionOperationsDbContext`, `ISystemAdminAccountService`, `IActivityPersistence`, `IRecentActivityQuery`, `IActivityHistoryQueryService`, and `IActivityReportService`; framework-managed transient hub/component instances. The consumer resolves scoped persistence through `IServiceScopeFactory`, so it has no captive `DbContext`. Assembly scanning/Scrutor was not added because the explicit registrations make lifetime and security boundaries visible.
