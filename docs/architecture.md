...
The admin boundary is deliberately small. Laravel owns the `users` table, standard password hashing, and the `access-admin` gate. It signs a short-lived RS256 token with a private key that stays in Laravel; Node receives only the public key and validates the signature, issuer, audience, expiry, and admin permission. Node then establishes a short-lived local HMAC-signed session using `LIVE_FEED_ADMIN_SESSION_SECRET`; the JWT is not stored in localStorage or a URL. The cookie is marked HttpOnly and SameSite, expires, and uses `Path=/` so the authenticated Socket.IO handshake at `/socket.io` receives it; it is Secure in production. Widening the cookie path does not make admin endpoints unauthenticated: HTTP routes and admin Socket.IO access remain server-side authorized. CSP, frame, content-type, referrer, and form-action protections are applied to the admin responses. The Node session remains valid until its short expiry, so this design does not claim immediate centralized token revocation. Production deployments still need TLS, secret management, CSRF, and rate-limiting controls.
...
### Laravel-owned admin identity for Live Feed Operations

The browser first authenticates with Laravel and follows the protected `GET /admin/live-feed` handoff. Laravel may issue the equivalent authenticated `POST /admin/live-feed/token` response, but the browser never stores the token persistently. The Node `POST /admin/auth/token` boundary accepts the short-lived bearer/form handoff, verifies it with the public key, and sets the existing root-scoped HttpOnly session cookie. The same cookie protects `/admin/live-feed`, `/admin/api/history`, `/admin/api/history.pdf`, and the server-side `admin:live-feed` Socket.IO authorization. Local key files are ignored; production private keys belong in a secret manager. Node has no admin password store and never receives Laravel password hashes.